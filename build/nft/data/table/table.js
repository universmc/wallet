...
async function generateTable() {
  try {
    ...
    const table = completion.choices[0]?.message?.content.split("\n").filter(Boolean);
    console.log("✅ Thématiques générées :", table);
    return table;
  } catch (error) {
...
async function generateLigne() {
  try {
    ...
    const Ligne = completion.choices[0]?.message?.content.split("\n").filter(Boolean);
    console.log("✅ Thématiques générées :", Ligne);
    return Ligne;
  } catch (error) {
    console.error("❌ Erreur lors de la génération des thématiques :", error);
    return [];
 ...
 */
async function generateVector(nodule) {
  try {
    const completion = await groq.chat.completions.create({
      messages: [
        { role: "assistant", content: `Génération de l'Vector en fonction de liste titre TOP-2024 fonction du "${nodule}"afin de garantir les prédictions les plus justes et affable possible pour le top 20255.` }
      ],
    ...
    console.log(`✅ Vector généré pour le thème "${nodule}" :`, VectorContent);
    return VectorContent;
  } catch (error) {
    console.error(`❌ Erreur lors de la génération de l'Vector pour "${nodule}" :`, error);
    return `Erreur lors de la génération de l'Vector pour le thème "${nodule}".`;
  }
}
...
async function main() {
  try {
    ...
    for (const nodule of table) {
      const VectorContent = await generateVector(nodule);

      // Enregistrement de l'Vector dans un fichier Markdown
      const outputFilePath = `AB2_${nodule.replace(/\s+/g, "_").toLowerCase()}_${new Date().toISOString().replace(/[-:TZ]/g, "")}.md`;
