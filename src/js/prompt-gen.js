...
async function generateThemes() {
  try {
    const completion = await groq.chat.completions.create({
      messages: [
        { role: "system", content: `Génération d'une liste de titres Dédié aux techniques aux méthodes d'apprentissage automatique : prompt --engine > ect ${autoPrompting}` }
      ],
      model: "gemma2-9b-it",
      temperature: 0.5,
...
async function main() {
  try {
    ...
    for (const theme of themes) {
      ...
      const outputFilePath = `prompt/engine_${theme.replace(/\s+/g, "_").toLowerCase()}_${new Date().toISOString().replace(/[-:TZ]/g, "")}.md`;
